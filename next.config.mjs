...
const nextConfig = {
  ...
  webpack: (config, { isServer }) => {
    ...
    // App Router-only project: pages-manifest.json is never generated by webpack
    // (it's a Pages Router artifact) but Next.js 14 reads it unconditionally
    // during static generation. Create an empty one after webpack emits so the
    // read doesn't throw ENOENT and abort the build.
    if (isServer) {
      config.plugins.push({
        apply(compiler) {

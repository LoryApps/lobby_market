...
export type FeedMode = "discover" | "following" | "foryou" | "mytags" | "unvoted" | "battleground" | "rising" | "closingin" | "newlaws" | "collapse" | "argued" | "flux" | "lastcall" | "momentum" | "mandate" | "elders" | "groundswell" | "livedebates" | "swing" | "stalled" | "comeback" | "overdrive" | "deadlock";
    ...
    (set, get) => ({
      ...
      fetchNextPage: async () => {
        ...
        try {
          if (feedMode === "following") {
          ...
          } else if (feedMode === "newlaws") {
            // New Laws feed — recently established laws ordered by passage date
            const params = new URLSearchParams({
              limit: "20",
              offset: String(offset),
              sort,
            ...
            const res = await fetch(`/api/feed/newlaws?${params.toString()}`);

            if (get()._generation !== capturedGen) return;

            if (!res.ok) {
              console.error("Failed to fetch new laws feed:", res.statusText);
              return;
            }
          ...
          } else if (feedMode === "collapse") {
            // Collapse feed — topics with the biggest FOR% drop in the last 7 days
            const params = new URLSearchParams({
              limit: "20",
              offset: String(offset),
              sort,
            ...
            const res = await fetch(`/api/feed/collapse?${params.toString()}`);

            if (get()._generation !== capturedGen) return;

            if (!res.ok) {
              console.error("Failed to fetch collapse feed:", res.statusText);
              return;
            }
          ...
          } else if (feedMode === "argued") {
            // Argued feed — topics ranked by argument activity in last 24h
            const params = new URLSearchParams({
              limit: "20",
              offset: String(offset),
              sort,
            ...
            const res = await fetch(`/api/feed/argued?${params.toString()}`);

            if (get()._generation !== capturedGen) return;

            if (!res.ok) {
              console.error("Failed to fetch argued feed:", res.statusText);
              return;
            }
          ...
          } else if (feedMode === "flux") {
            // Flux feed — topics with the highest consensus shift in 24h
            const params = new URLSearchParams({
              limit: "20",
              offset: String(offset),
            });

            const res = await fetch(`/api/feed/flux?${params.toString()}`);

            if (get()._generation !== capturedGen) return;

            if (!res.ok) {
              console.error("Failed to fetch flux feed:", res.statusText);
              return;
            }
          ...
          } else if (feedMode === "lastcall") {
            // Last Call feed — voting topics ordered by voting_ends_at ascending
            const params = new URLSearchParams({
              limit: "20",
              offset: String(offset),
            });

            const res = await fetch(`/api/feed/lastcall?${params.toString()}`);
          ...
          } else if (feedMode === "mandate") {
            // Mandate feed — topics with overwhelming consensus (≥80% FOR or AGAINST)
            const params = new URLSearchParams({
              limit: "20",
              offset: String(offset),
              sort,
            ...
            const res = await fetch(`/api/feed/mandate?${params.toString()}`);

            if (get()._generation !== capturedGen) return;

            if (!res.ok) {
              console.error("Failed to fetch mandate feed:", res.statusText);
              return;
            }
          ...
          } else if (feedMode === "elders") {
            // Elders feed — topics that have been unresolved for 30+ days
            const params = new URLSearchParams({
              limit: "20",
              offset: String(offset),
              sort,
            ...
            const res = await fetch(`/api/feed/elders?${params.toString()}`);

            if (get()._generation !== capturedGen) return;

            if (!res.ok) {
              console.error("Failed to fetch elders feed:", res.statusText);
              return;
            }
          ...
          } else if (feedMode === "groundswell") {
            // Groundswell feed — previously quiet topics suddenly spiking in activity
            const params = new URLSearchParams({
              limit: "20",
              offset: String(offset),
            });

            const res = await fetch(`/api/feed/groundswell?${params.toString()}`);

            if (get()._generation !== capturedGen) return;

            if (!res.ok) {
              console.error("Failed to fetch groundswell feed:", res.statusText);
              return;
            }
          ...
          } else if (feedMode === "swing") {
            // Swing feed — topics where recent votes reverse the overall consensus
            const params = new URLSearchParams({
              limit: "20",
              offset: String(offset),
            });

            const res = await fetch(`/api/feed/swing?${params.toString()}`);

            if (get()._generation !== capturedGen) return;

            if (!res.ok) {
              console.error("Failed to fetch swing feed:", res.statusText);
              return;
            }
          ...
          } else if (feedMode === "stalled") {
            // Stalled feed — debates that went quiet after a burst of activity
            const params = new URLSearchParams({
              limit: "20",
              offset: String(offset),
              sort,
            ...
            const res = await fetch(`/api/feed/stalled?${params.toString()}`);

            if (get()._generation !== capturedGen) return;

            if (!res.ok) {
              console.error("Failed to fetch stalled feed:", res.statusText);
              return;
            }
          ...
          } else if (feedMode === "comeback") {
            // Comeback feed — debates that went dormant but just revived
            const params = new URLSearchParams({
              limit: "20",
              offset: String(offset),
              sort,
            ...
            const res = await fetch(`/api/feed/comeback?${params.toString()}`);

            if (get()._generation !== capturedGen) return;

            if (!res.ok) {
              console.error("Failed to fetch comeback feed:", res.statusText);
              return;
            }
      ...
      setFeedMode: (feedMode) => {
        ...
        const sort = (feedMode === "following" || feedMode === "mytags") ? "new"
          ...
          : (feedMode === "rising" || feedMode === "closingin" || feedMode === "newlaws" || feedMode === "collapse" || feedMode === "argued" || feedMode === "flux" || feedMode === "lastcall" || feedMode === "mandate" || feedMode === "elders" || feedMode === "groundswell" || feedMode === "livedebates" || feedMode === "swing" || feedMode === "stalled" || feedMode === "comeback" || feedMode === "overdrive" || feedMode === "deadlock") ? "top"
          : "top";
        set({
          feedMode,
          sort,

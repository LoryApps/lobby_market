...
/**
 * /leaderboard/year — Annual Civic Leaderboard
 *
 * Ranks citizens by civic contributions across a full calendar year.
 * Four ranked views:
 *   Voters       — most votes cast this year
 ...
 *
 * Navigate backward/forward with year arrows. Defaults to current year.
 *
 * Distinct from:
 *   /leaderboard/today   — single day
 *   /leaderboard/week    — current ISO week
 *   /leaderboard/month   — single calendar month
...
import type {
  AnnualLeaderboardResponse,
  AnnualEntry,
  AnnualMyRanks,
} from '@/app/api/leaderboard/annual/route'
...
function MyRankCard({ ranks, activeTab }: { ranks: AnnualMyRanks; activeTab: string }) {
  const stat = {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 text-sm font-mono text-surface-400">Your rank this year</div>
...
function AnnualLeaderboardInner() {
  ...
  const [data, setData] = useState<AnnualLeaderboardResponse | null>(null)
  ...
  const load = useCallback(async (y: number) => {
    ...
    setError(false)
    try {
      const res = await fetch(`/api/leaderboard/annual?year=${y}`, {
        signal: ctrl.signal,
        cache: 'no-store',
      })
  ...
  function navigate(delta: number) {
    ...
    if (nextYear < 2020 || nextYear > currentYear) return
    router.replace(`/leaderboard/year?year=${nextYear}`)
  }

  const isCurrentYear = year === now.getFullYear()
  const tabConfig = TABS.find((t) => t.id === tab)!

  const entries: AnnualEntry[] = data
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 pb-24 md:pb-12" id="main-content">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-mono text-surface-500 mt-0.5">
                  Top civic contributors for a full calendar year
                </p>
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex-1 text-center">
            <p className="font-mono text-2xl font-bold text-white tracking-wide">{year}</p>
            {isCurrentYear && (
        ...
        <div
          ...
          className="grid grid-cols-4 gap-1.5 mb-6 bg-surface-200/50 p-1.5 rounded-xl border border-surface-300/40"
        >
          {TABS.map((t) => {
            const Icon = t.icon
            ...
            return (
              <button
                ...
                onClick={() => setTab(t.id)}
                className={cn(
                  'flex flex-col items-center gap-0.5 py-2 px-1 rounded-lg text-[11px] font-mono font-semibold transition-all',
                  isActive
                    ? `${t.activeBg} border ${t.border} ${t.color} shadow-sm`
        ...
        {/* ── Error ───────────────────────────────────────────────────────── */}
        {error && (
          <div className="text-center py-8 text-against-400 font-mono text-sm">
            Failed to load leaderboard.{' '}
            <button
              onClick={() => load(year)}
              className="underline hover:text-against-300 transition-colors"
            >
              Retry
            </button>
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {loading ? (
              ...
              <EmptyState
                icon={tabConfig.icon}
                title={`No ${tabConfig.label.toLowerCase()} yet`}
                description={`No civic activity recorded for ${year} in this category.`}
              />
            ) : (
              entries.map((entry, i) => (
                <EntryRow
        ...
        {!loading && (
          <div className="mt-8 pt-6 border-t border-surface-300/40">
            <p className="text-xs font-mono text-surface-500 mb-3 uppercase tracking-widest">Other time ranges</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[
                { href: '/leaderboard/today', label: 'Today', color: 'text-for-400' },
                { href: '/leaderboard/week', label: 'This week', color: 'text-purple' },
                { href: '/leaderboard/month', label: 'This month', color: 'text-emerald' },
                { href: '/leaderboard', label: 'All-time', color: 'text-gold' },
              ].map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={cn(
                    'flex items-center justify-center py-2 px-3 rounded-xl text-xs font-mono font-semibold',
                    'bg-surface-200 border border-surface-300/60 hover:border-surface-400/60',
                    'transition-colors',
                    link.color
                  )}
                >
...
export default function AnnualLeaderboardPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-surface-50">

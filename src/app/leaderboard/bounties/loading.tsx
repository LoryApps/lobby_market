      ...
      <main className="max-w-3xl mx-auto px-4 pt-6 pb-24 md:pb-12">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[1, 0, 2].map((rank) => (
            <div
              key={rank}
              className={`rounded-2xl bg-surface-100 border border-surface-300 p-4 flex flex-col items-center gap-2 ${
                rank === 0 ? 'ring-1 ring-gold/30' : ''
              }`}
            >
              <Skeleton
                className={`rounded-full flex-shrink-0 ${
                  rank === 0 ? 'h-14 w-14' : 'h-11 w-11'
                }`}
              />

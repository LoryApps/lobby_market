...
const CHAPTERS: Chapter[] = [
  ...
  },
  {
    ...
    subtitle: 'Four-dimension cognitive bias report — confirmation, social proof, negativity, and category tunnel vision',
    accent: 'text-against-400',
    borderColor: 'border-against-500/30',
    bgColor: 'bg-against-500/5',
    textColor: 'text-against-400',
    items: [
      { icon: Brain, label: '/bias: four arc-gauge metrics — Confirmation Bias (upvote/vote-side alignment), Social Proof Bias (% votes with majority), Negativity Bias (FOR/AGAINST imbalance), Category Tunnel Vision (Shannon entropy of category spread). Composite overall bias score with label and colour.', href: '/bias', color: 'text-against-400' },
      { icon: Shield, label: 'Expandable detail cards per bias type with your evidence (raw numbers) plus actionable tips to broaden civic engagement. SVG arc gauges with animated fill.', href: '/bias', color: 'text-surface-400' },
      { icon: Activity, label: 'New /api/analytics/bias route — zero new DB schema, uses existing votes + argument_upvotes tables. Added to sitemap and ⌘K Command Palette.', href: '/bias', color: 'text-surface-400' },
    ],
  },
  {
    number: 'Ch. 214',
    title: 'The Civic Epoch',
    subtitle: 'Platform history told through named civic epochs — each era defined by its dominant debates and milestones',
    accent: 'text-gold',
    borderColor: 'border-gold/30',
    bgColor: 'bg-gold/5',
    textColor: 'text-gold',
    items: [
      { icon: History, label: '/epoch: platform history divided into named civic epochs, each defined by its dominant debates, milestones, and shifts in collective opinion. Epoch cards show date range, key topics, and defining moment.', href: '/epoch', color: 'text-gold' },
      { icon: CalendarClock, label: 'Epoch timeline with interactive navigation. Each epoch captures the platform\'s civic mood at that moment — what the community was debating, what became law, and what was rejected.', href: '/epoch', color: 'text-surface-400' },
      { icon: Activity, label: 'Added to sitemap. New /api/epoch route aggregates topic history by time period and assigns epoch labels based on dominant themes.', href: '/epoch', color: 'text-surface-400' },
    ],
  },
  {
    number: 'Ch. 215',
    title: 'The Civic Zeitgeist',
    subtitle: 'Platform-wide mood board — what the collective is feeling, debating, and deciding right now',
    accent: 'text-purple',
    borderColor: 'border-purple/30',
    bgColor: 'bg-purple/5',
    textColor: 'text-purple',
    items: [
      { icon: Sparkles, label: '/zeitgeist: a live mood snapshot of the platform — the emotional and civic temperature of all active debates right now. Aggregates sentiment, momentum, and participation signals into a single mood reading.', href: '/zeitgeist', color: 'text-purple' },
      { icon: Radio, label: 'Mood dimensions: civic energy (vote velocity), consensus vs. conflict (average margin), civic gravity (topics trending toward law), and discourse health (argument-to-vote ratio).', href: '/zeitgeist', color: 'text-surface-400' },
      { icon: Activity, label: 'Added to sitemap. New /api/zeitgeist route computes all mood dimensions from recent activity in real time.', href: '/zeitgeist', color: 'text-surface-400' },
    ],
  },
  {
    number: 'Ch. 216',
    title: 'Civic Volatility Index',
    subtitle: 'Topics ranked by opinion swing intensity — which debates are flipping the fastest',
    accent: 'text-against-400',
    borderColor: 'border-against-500/30',
    bgColor: 'bg-against-500/5',
    textColor: 'text-against-400',
    items: [
      { icon: Activity, label: '/volatility: ranks active topics by how dramatically opinion has swung in recent days. Volatility score = standard deviation of daily FOR% across a rolling 7-day window.', href: '/volatility', color: 'text-against-400' },
      { icon: TrendingDown, label: 'Each topic card shows the volatility score, current FOR%, 7-day swing range (min/max), and a sparkline of daily opinion. Tier labels: Stable → Shifting → Volatile → Explosive.', href: '/volatility', color: 'text-surface-400' },
      { icon: Activity, label: 'New /api/volatility route fetches recent votes per topic, computes daily FOR% timeseries, and ranks by standard deviation. Added to sitemap.', href: '/volatility', color: 'text-surface-400' },
    ],
  },
  {
    number: 'Ch. 217',
    title: 'Civic Depth Index',
    subtitle: 'Debates ranked by intellectual richness — argument density, citation rate, AI quality, wiki coverage, predictions, and reply depth',
    accent: 'text-for-400',
    borderColor: 'border-for-500/30',
    bgColor: 'bg-for-500/5',
    textColor: 'text-for-400',
    items: [
      { icon: Brain, label: '/depth: composite 0–100 Depth Score per topic. Six dimensions — argument density (30pts), citation rate (20pts), AI quality score (25pts), wiki richness (15pts), prediction engagement (10pts), reply depth (10pts). Expandable rows show animated dimension bars.', href: '/depth', color: 'text-for-400' },
      { icon: BarChart2, label: 'Category breakdown cards, sort options (depth score / argument density / citation rate / argument count), search filter, and info panel explaining the scoring formula. New /api/depth route aggregates all six signals from existing tables — zero new migrations.', href: '/depth', color: 'text-surface-400' },
...
const RECENT_BUILDS: RecentBuild[] = [
  ...
  {
    title: 'Debate Calendar iCal Subscription Feed',
    description: 'New /api/debates/calendar.ics feed (Ch. 301) — one-click iCal subscription URL that outputs all scheduled and live debates as RFC 5545 calendar events. Each event includes debate title, type, topic statement, participant usernames, and a direct link to the arena. Lobby-branded PRODID. Linked from the Debate Calendar page via a new "Subscribe with iCal" button. The individual-debate iCal export (/api/debates/[id]/ics) was already present; this adds the platform-wide feed for subscribing to ALL upcoming debates at once.',
  ...
  {
    title: 'Pre-Debate AI Coaching Brief',
    description: 'New /debate/[id]/coach page (Ch. 300) — registered debate participants get a personalised AI-generated coaching brief. The brief surfaces: their assigned side, their opponent\'s civic profile and debate record, the top FOR/AGAINST arguments on the topic, an AI-generated opening hook + 3 core points + closing line, and a tactical tip tailored to the debate format. Backed by /api/debate/[id]/coach route using Claude claude-sonnet-4-6. Linked from the DebateArena pre-start overlay when the user is a registered participant.',
    href: '/debate',
    icon: Sparkles,
    color: 'text-gold',
    tag: 'Ch. 300',
  ...
  {
    title: 'Debate Verdict Page',
    description: 'New /debate/[id]/verdict page (Ch. 299) — comprehensive post-debate verdict view. Shows final sway bar, winner declaration (FOR / AGAINST / Tie based on community winner poll), topic impact summary (vote shift before vs. after), and participant performance highlights. Prominent share card for the verdict. Linked from DebateArena ended overlay and DebateRecap. Fetches from /api/debates/[id]/verdict route.',
    href: '/debate',
    icon: Trophy,
    color: 'text-gold',
    tag: 'Ch. 299',
  ...
  {
    title: 'Debate Audience Chamber',
    description: 'New /debate/[id]/audience page (Ch. 298) — full audience experience view for ended debates. Shows RSVP vs actual viewer count, the community "who argued better?" winner poll (DebateWinnerPoll component), sway progression checkpoints across the debate, and the final topic impact. Distinct from the live audience meter in DebateArena — this is a post-debate audience analytics view. Backed by /api/debates/[id]/audience route.',
    href: '/debate',
    icon: Users,
    color: 'text-for-400',
    tag: 'Ch. 298',
  ...
  {
    title: 'Debate Highlight Reel',
    description: 'New /debate/[id]/highlights page (Ch. 297) — surfaced the top arguments from any ended debate, ranked by upvotes + sway contribution. Each highlight card shows the argument, author, side pill, upvote count, and a "this argument swayed X% of the audience" sway badge. Backed by /api/debates/[id]/highlights route. Linked from DebateArena ended overlay and DebateRecap action row.',
    href: '/debate',
    icon: Star,
    color: 'text-gold',
    tag: 'Ch. 297',
  ...
  {
    title: 'Blueprint Community Notes',
    description: 'New annotation layer on law implementation blueprints (Ch. 293) — citizens can add, read, upvote, and delete notes on any law\'s AI-generated blueprint. BlueprintNotes component embedded in /law/[id]/blueprint. Notes are scoped to the law (not the cached blueprint row) so they survive regeneration. DB migration 00088_blueprint_notes.sql: blueprint_notes table with law_id, author_id, content (1–800 chars), upvote_count. RLS policies for public read / auth insert / own delete. /api/laws/[id]/blueprint-notes route for GET + POST.',
    href: '/law',
    icon: MessageSquare,
    color: 'text-purple',
    tag: 'Ch. 293',
  ...
  {
    title: 'Law Implementation Blueprint',
    description: 'New /law/[id]/blueprint page (Ch. 292) — AI-generated implementation plans for every established Codex law. Claude generates a structured plan covering: implementation phases with timelines, key stakeholders, estimated resource requirements, likely challenges, measurable success metrics, and international policy comparisons. Results are cached in DB migration 00087_law_blueprints.sql and regenerated on demand. Overall outlook rating (Promising/Achievable/Complex/Challenging). Linked from main law page with a purple Blueprint button.',
    href: '/law',
    icon: Layers,
    color: 'text-purple',
    tag: 'Ch. 292',
  ...
  {
    title: 'iOS Prediction Market View',
    description: 'iOS app Ch. 291 — native PredictionView.swift: browse and place predictions on topic outcomes directly from the iOS app. Prediction cards show question, your predicted probability, current market probability, resolution status, clout staked, and potential return. Resolve mechanic for ended predictions with Brier score display. SupabaseClient extended with fetchPredictions() and submitPrediction(). PredictionDetail sheet with probability slider and stake input. Linked from TopicDetailView\'s Predict button.',
    icon: Smartphone,
    color: 'text-emerald',
    tag: 'Ch. 291',
  },
  {
    title: 'iOS Vote Activity Heatmap',
    description: 'iOS app Ch. 290 — VoteHeatmapView.swift: GitHub-style contribution heatmap showing daily vote activity for the past 52 weeks. Each cell is colour-coded by vote count (0 = darkest surface, 4+ = brightest blue). Integrated into both the own ProfileView and the public PublicProfileView so you can compare your cadence with other citizens\'. Month labels, weekday labels, and a legend strip. Data from a new /api/me/vote-heatmap endpoint returning daily counts for the last 365 days.',
    icon: Smartphone,
    color: 'text-for-400',
    tag: 'Ch. 290',
  },
  {
    title: 'iOS CategoryDetailView',
    description: 'iOS app Ch. 289 — full-screen CategoryDetailView.swift replacing the web Safari link for category browsing. Tapping a category on the DiscoverView opens a native sheet with: category hero banner (icon + colour), stat strip (topic count, law count, active debates), paginated topic list with vote bars and status pills, and a "Browse all" button linking to /categories/{category} in Safari. SupabaseClient.fetchCategoryTopics(category:) added — queries topics table filtered by category, ordered by total_votes descending.',
    icon: Smartphone,
    color: 'text-for-400',
    tag: 'Ch. 289',
  },
  {
    title: 'iOS Siri App Intents',
    description: 'iOS app Ch. 288 — SiriKit App Intents integration. Four custom intents: GetTopTopicIntent (ask Siri "What\'s trending in the Lobby?"), GetMyStreakIntent ("What\'s my voting streak?"), GetRecentLawIntent ("What was the last law passed?"), CastVoteIntent (deep-link to topic voting). AppIntents.swift registers all four with Siri and Shortcuts app. IntentHandler.swift bridges Supabase fetches to intent responses. Xcode project.pbxproj updated with Intent definition file and handler group. Info.plist NSUserActivityTypes updated.',
    icon: Smartphone,
    color: 'text-purple',
    tag: 'Ch. 288',
  },
  {
    title: 'iOS Public Profile View + Follow',
    description: 'iOS app Ch. 287 — PublicProfileView.swift: tap any citizen across the app to open their full public profile. Shows avatar initials, display name, username, role badge, clout, vote count, reputation, bio, top arguments, and recent vote activity. Follow/Unfollow button with optimistic state (green when following, white when not). SupabaseClient extended with fetchPublicProfile(username:) and toggleFollow(targetId:). Follow count displayed in the profile header. Tapping leaderboard entries now opens PublicProfileView instead of Safari.',
    icon: Smartphone,
    color: 'text-for-400',
    tag: 'Ch. 287',
  },
  {
    title: 'iOS Discover & Vote Heatmap',
    description: 'iOS app Ch. 286 — two features. (1) DiscoverView.swift: curated discovery hub with three sections — Top Categories (tap to filter topics), Recommended Topics (personalised by vote history), and Rising Citizens (follow suggestions). Category chips with icons and colour accents. Pull-to-refresh. (2) VoteHeatmapCell + HeatmapRow components extracted as reusable SwiftUI views ahead of profile integration. SupabaseClient.fetchDiscoverData() added. New Explore tab (compass.fill) added to ContentView between Trends and Codex.',
    icon: Smartphone,
    color: 'text-emerald',
    tag: 'Ch. 286',
  },
  {
    title: 'iOS Argument Threading',
    description: 'iOS app Ch. 285 — native reply threads on arguments. ArgumentRow.tsx now shows a Reply button that opens ReplySheet.swift — a compose sheet for threaded replies with 300-char limit and FOR/AGAINST side indicator. RepliesView.swift lists all child replies for an argument with avatar, username, content, side pill, and upvote count. ThreadedArgumentRow wraps ArgumentRow + expandable RepliesView. SupabaseClient extended with fetchReplies(argumentId:) and postReply(). Upvote haptic feedback on both arguments and replies (UIImpactFeedbackGenerator, medium style).',
    icon: Smartphone,
    color: 'text-purple',
    tag: 'Ch. 285',
  },
  {
    title: 'iOS Trending Now View',
    description: 'iOS app Ch. 284 — TrendingView.swift: a dedicated Trending tab (chart.bar.fill icon) showing three ranked lists: Hot Topics (by vote velocity in last 24 h), New Laws (most recently established), and Top Arguments (most upvoted across the platform today). Each section has a "See all" chevron linking to the corresponding web page in Safari. TrendingTopic, TrendingLaw, TrendingArgument Swift models. SupabaseClient.fetchTrending() aggregates all three lists in a single call. Skeleton loading states per section. Replaced the placeholder Trends tab.',
    icon: Smartphone,
    color: 'text-for-400',
    tag: 'Ch. 284',
  },
  {
    title: 'iOS Push Notification System',
    description: 'iOS app Ch. 283 — end-to-end push notification pipeline. UNUserNotificationCenter.requestAuthorization on first launch. APNs device token registration via /api/push/register-apns (new route, stores in DB migration 00086_apns_tokens.sql). Notification payload handler routes taps to the correct screen: debate_starting → DebateDetailView, law_established → LawDetailView, achievement_earned → AchievementsView, reply_received → TopicDetailView. Background fetch mode enabled in entitlements. NotificationDelegate.swift implements UNUserNotificationCenterDelegate.',
    icon: Smartphone,
    color: 'text-against-400',
    tag: 'Ch. 283',
  },
  {
    title: 'iOS Clout Wallet + Daily Briefing',
    description: 'iOS app Ch. 282 — two high-value screens. (1) CloutWalletView.swift: full clout economy dashboard. Balance display with animated number, weekly earned/spent bars, recent transaction log (earn/spend with icon per action type), and link to transfer clout. SupabaseClient.fetchCloutHistory() added. (2) DailyBriefingView.swift: personalised start-of-day screen surfacing today\'s vote progress, streak status, upcoming debates, and a featured topic in the user\'s top category. Accessible from the home tab pull-to-refresh prompt.',
    icon: Smartphone,
    color: 'text-gold',
    tag: 'Ch. 282',
  },
  {
    title: 'iOS Topic Wiki View',
    description: 'iOS app Ch. 281 — WikiView.swift: read topic wiki articles natively without opening Safari. Parses the stored markdown description with a lightweight renderer supporting headers (# ##), bold (**text**), italic (*text*), bullet lists, numbered lists, blockquotes, inline code, and [[wikilink]] cross-references (highlighted in blue, tap to open the linked topic). Shows last-edited timestamp and editor username. Edit button deep-links to the web wiki editor. SupabaseClient.fetchWiki(topicId:) added. Embedded as a tab inside TopicDetailView.',
    icon: Smartphone,
    color: 'text-for-300',
    tag: 'Ch. 281',
  },
  {
    title: 'iOS Coalition Posts & Members',
    description: 'iOS app Ch. 280 — CoalitionDetailView.swift: full coalition hub screen. Header with coalition name, description, member count, influence score, and join/leave button. Two tabs: Posts (bulletin-board messages from members, with upvote and reply counts) and Members (ranked by clout, with role badge, follow button). CoalitionPost and CoalitionMember Swift models. SupabaseClient extended with fetchCoalitionPosts() and fetchCoalitionMembers(). Pull-to-refresh on both tabs. Tapping a member opens PublicProfileView.',
    icon: Smartphone,
    color: 'text-purple',
    tag: 'Ch. 280',
  },
  {
    title: 'iOS Feed Category Filters + Tags',
    description: 'iOS app Ch. 279 — two improvements to the iOS feed. (1) Category filter chips rendered as a horizontally-scrollable row above the topic list. Tapping a chip filters topics to that category (All / Economics / Politics / Technology / …). Active chip highlighted in blue. (2) TagBrowserView.swift: full-screen tag explorer showing trending tags with topic counts and colour accents. Tapping a tag opens a filtered topic list. TagRow component with topic-count badge. SupabaseClient.fetchTrendingTags() added. TagBrowser accessible via search bar accessory button.',
    icon: Smartphone,
    color: 'text-for-400',
    tag: 'Ch. 279',
  },
  {
    title: 'iOS Bookmarks System',
    description: 'iOS app Ch. 278 — BookmarksView.swift: full bookmarks hub with four tabs (Topics, Laws, Arguments, Collections). Each tab shows a paginated list of bookmarked items with the relevant metadata (vote bar for topics, establishment date for laws, upvote count for arguments). Swipe-left to remove a bookmark. Empty state with CTA to the feed. SupabaseClient extended with fetchBookmarks(type:), addBookmark(type:id:), and removeBookmark(type:id:). Bookmark button added to TopicDetailView and LawDetailView long-press context menus. DB reads from topic_bookmarks and argument_bookmarks tables.',
    icon: Smartphone,
    color: 'text-for-400',
    tag: 'Ch. 278',
  },
  {
    title: 'iOS Direct Messaging',
    description: 'iOS app Ch. 277 — MessagesView.swift + ConversationView.swift: full private messaging system. Inbox lists all conversations sorted by last message, with unread badge, avatar, and preview text. Tapping opens ConversationView: a full chat-style thread with message bubbles (blue = sent, surface = received), timestamps, and a compose bar with send button. New message button opens a citizen search to start a conversation. Real-time polling every 10 s in open conversations. SupabaseClient extended with fetchInbox(), fetchMessages(partnerId:), and sendMessage(). Reads from direct_messages table.',
    icon: Smartphone,
    color: 'text-for-400',
    tag: 'Ch. 277',
  },
  {
    title: 'iOS Prediction Market',
    description: 'iOS app Ch. 276 — native prediction market screens. PredictionsListView.swift shows all open predictions with probability slider, current market probability bar, and stake input. PredictionCard component with colour-coded probability arc (green = high confidence FOR, red = high confidence AGAINST). My Predictions tab shows your active bets with potential return calculations. ResolvedPredictionCard shows outcome, your prediction, Brier score, and clout earned/lost. SupabaseClient.fetchMarketPredictions() and submitPrediction() added. Accessible via Lobby tab.',
    icon: Smartphone,
    color: 'text-emerald',
    tag: 'Ch. 276',
  },
  {
    ...
    description: 'iOS app Ch. 275 — native onboarding for new iOS users. OnboardingView.swift: 5-screen flow. Welcome splash (Lobby Market logo + tagline). 5-binary calibration quiz (same questions as the web onboarding) with left/right swipe gesture to answer. Results screen showing top 2 recommended categories based on answers. Notification permission prompt (UNUserNotificationCenter). Final "Start voting" CTA leading to the main feed. Progress dots indicator. Each screen uses spring animations. First-launch check via UserDefaults. OnboardingComplete stored in Supabase profile via PATCH /api/me.',
    icon: Smartphone,
    color: 'text-gold',
    tag: 'Ch. 275',
  },
  {
    title: 'iOS DebateDetailView + RSVP',
    description: 'iOS app Ch. 274 — DebateDetailView.swift: full debate detail screen opened from DebatesView. Shows title, type badge (Oxford/Panel/Town Hall), status pill (Scheduled/Live/Ended), scheduled time countdown, description, FOR/AGAINST sway bars, participant list with role badges (Proposer/Opposition/Chair/Panelist), and viewer count. RSVP button (green = attending, outlined = not attending) with optimistic toggle and haptic feedback. SupabaseClient extended with fetchDebateDetail(id:) and toggleRsvp(debateId:). Live debates show a pulsing red "LIVE" indicator.',
    icon: Smartphone,
    color: 'text-against-400',
    tag: 'Ch. 274',
  },
  {
    title: 'iOS Argument Arena',
    description: 'iOS app Ch. 273 — ArenaView.swift: native argument quality judging. Shows two arguments head-to-head (FOR on left, AGAINST on right) for the same topic. User taps the stronger argument to cast a judgment. Animated reveal of result with win/lose flash. Running score strip. Skip button for arguments you can\'t fairly judge. ArenaResult sheet shows which side "won" the round and a quality score. SupabaseClient.fetchArenaArguments() returns paired FOR/AGAINST arguments from the same topic. submitArenaVote() records the judgment. Accessible from TopicDetailView\'s Arena tab.',
    icon: Smartphone,
    color: 'text-against-400',
    tag: 'Ch. 273',
  },
  {
    title: 'iOS SettingsView',
    description: 'iOS app Ch. 272 — full SettingsView.swift with five sections. Account (display name, username, email — read-only with edit deep-link). Notifications (toggle per-type: achievements, debates, laws, replies, streaks, weekly digest) stored in UserDefaults and synced to /api/me/notification-prefs. Appearance (system / light / dark colour scheme preference). About (app version, open-source credits, privacy policy, terms links). Danger Zone (sign out button with confirmation alert). Accessible via profile tab gear icon. NotificationPref model mirrors web notification_prefs structure.',
    icon: Smartphone,
    color: 'text-surface-400',
    tag: 'Ch. 272',
  },
  {
    title: 'iOS Achievements View + API',
    description: 'iOS app Ch. 271 — AchievementsView.swift: browse your full achievement collection. Three tabs: Earned (unlocked, with date and tier badge), In Progress (locked with % completion bar), and All (grid view of every achievement). Each card shows icon, title, tier (Common/Rare/Epic/Legendary) with colour accent, and a short description. Legendary achievements glow with a gold ring animation. New /api/achievements/mine endpoint returns both earned and in-progress achievements with completion percentages computed server-side. AchievementRow and AchievementCard SwiftUI components.',
    icon: Smartphone,
    color: 'text-gold',
    tag: 'Ch. 271',
  },
  {
    title: 'iOS Coalitions/Lobbies View',
    description: 'iOS app Ch. 270 — CoalitionsView.swift: browse and join political alliances from the iOS app. Two tabs: My Coalitions (coalitions you\'ve joined with your role and recent activity) and Discover (all public coalitions ranked by influence). CoalitionCard shows name, description, member count, influence score, category tags, and a Join/Leave button with confirmation alert. CoalitionSummary Swift model. SupabaseClient extended with fetchCoalitions(), fetchMyCoalitions(), and toggleCoalitionMembership(id:). New Lobby tab added to ContentView (building.2.fill icon) after Codex.',
    icon: Smartphone,
    color: 'text-purple',
    tag: 'Ch. 270',
  },
  {
    title: 'iOS Rich ProfileView',
    description: 'iOS app Ch. 269 — ProfileView.swift fully rebuilt with a rich multi-section layout. Hero section: large avatar initials circle, display name, username, role badge (Citizen/Debator/Elder/Troll Catcher). Stats grid: Clout (gold), Votes (blue), Arguments (purple), Reputation (emerald). Civic Archetype card (AI-generated political archetype with colour accent). Bio text with markdown support. Top Arguments list (3 most-upvoted, with FOR/AGAINST side pill). Vote category donut chart (top 3 categories by vote count). Streak status with flame icon. SupabaseClient.fetchRichProfile() aggregates all profile data.',
    icon: Smartphone,
    color: 'text-for-400',
    tag: 'Ch. 269',
  },
  {
    title: 'iOS Civic Stats View',
    description: 'iOS app Ch. 268 — CivicStatsView.swift: personal civic statistics dashboard. Six KPI cards: Total Votes, Vote Streak, Arguments Written, Debates Participated, Laws Influenced (voted FOR on topics that became law), and Accuracy Score (% of resolved votes that matched the eventual outcome). Each card has an icon, large number with animated counter, and a comparison label ("above platform avg", "personal best", etc.). Monthly vote activity bar chart (last 6 months). Top categories ranked by vote count. SupabaseClient.fetchCivicStats() endpoint added. Accessible via a new Stats button in ProfileView.',
    icon: Smartphone,
    color: 'text-for-400',
    tag: 'Ch. 268',
  },
  ...
  {
    title: 'Civic Volatility Index',
    description: 'New /volatility page ranking active topics by how dramatically opinion has swung in recent days. Volatility score = standard deviation of daily FOR% across a rolling 7-day window. Four tiers: Stable → Shifting → Volatile → Explosive. Each card shows the volatility score, current FOR%, 7-day swing range (min/max), and a sparkline of daily opinion movement. Category filter. New /api/volatility route fetches recent votes per topic, computes daily FOR% timeseries, and ranks by standard deviation.',
    href: '/volatility',
    icon: Activity,
    color: 'text-against-400',
    tag: 'Ch. 216',
  ...
  {
    title: 'The Civic Zeitgeist',
    description: 'New /zeitgeist page: a live mood snapshot of the platform — the emotional and civic temperature of all active debates right now. Aggregates sentiment, momentum, and participation signals into a single mood reading. Mood dimensions: civic energy (vote velocity), consensus vs. conflict (average margin), civic gravity (topics trending toward law), and discourse health (argument-to-vote ratio). New /api/zeitgeist route computes all mood dimensions from recent activity in real time.',
    href: '/zeitgeist',
    icon: Sparkles,
    color: 'text-purple',
    tag: 'Ch. 215',
  ...
  {
    title: 'The Civic Epoch',
    description: 'New /epoch page: platform history divided into named civic epochs, each defined by its dominant debates, milestones, and shifts in collective opinion. Epoch cards show date range, key topics, and the defining moment for each era. Interactive timeline navigation. Captures the platform\'s civic mood at each historical moment — what the community was debating, what became law, and what was rejected. New /api/epoch route aggregates topic history by time period and assigns epoch labels based on dominant themes.',
    href: '/epoch',
    icon: History,
    color: 'text-gold',
    tag: 'Ch. 214',

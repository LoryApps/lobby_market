  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center gap-2 p-6 rounded-2xl bg-surface-100 border border-surface-300/50">
        ...
        <span className="text-2xl font-bold text-white">{score}<span className="text-sm text-surface-500 font-normal">/{MAX_SCORE}</span></span>
...
export default function GaugePage() {
  ...
  // Load saved state or fetch questions
  useEffect(() => {
    try {
    ...
    return (
      <div className="min-h-screen bg-surface-50 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <Gauge className="h-8 w-8 text-gold animate-pulse" />
        </div>
        <BottomNav />
      </div>
    )
                ...
                {!revealed ? (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-xs text-surface-500 pt-1">
                      <span>Running total: <span className="text-white font-bold">{totalScore + (guesses[guesses.length - 1]?.pts ?? 0)} pts</span></span>
                    </div>

...
export default function SynthesisError({ reset }: { reset: () => void }) {
  return (
    <div className="flex flex-col min-h-screen bg-surface-50">
      <TopBar />
      <main className="flex-1 flex flex-col items-center justify-center gap-4 px-4">
        <AlertTriangle className="h-10 w-10 text-against-400" />
        <h1 className="font-mono text-lg font-bold text-white">Failed to load Synthesis Hub</h1>
        <p className="font-mono text-sm text-surface-500 text-center max-w-sm">
          Could not retrieve synthesis data. Try refreshing the page.
        </p>

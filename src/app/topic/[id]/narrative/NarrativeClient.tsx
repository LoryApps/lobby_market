...
/**
 * /topic/[id]/narrative — Civic Narrative Arc
 *
 * An AI-generated journalistic story of a debate: how it started, what
 * arguments emerged, and where consensus currently stands.
 *
 * Distinct from:
 *   /brief      — neutral voter briefing (3 short paragraphs, factual)
 *   /synthesis  — structured comparative analysis (panel format)
 *   /steelman   — strongest case for each side (advocacy format)
 *   /recap      — retrospective for concluded debates (stats-heavy)
 *
 * This is narrative journalism: characters, stakes, tension, story.
 */
...
import type { NarrativeResponse, NarrativeChapter } from '@/app/api/topics/[id]/narrative/route'
...
const STATUS_CONFIG: Record<string, { label: string; badge: 'proposed' | 'active' | 'law' | 'failed'; icon: typeof Scale; color: string }> = {
  proposed:  { label: 'Proposed',       badge: 'proposed', icon: Scale,   color: 'text-surface-400' },
  active:    { label: 'Active',          badge: 'active',   icon: Zap,     color: 'text-for-400' },
  voting:    { label: 'Voting',          badge: 'active',   icon: Zap,     color: 'text-gold' },
  law:       { label: 'Established Law', badge: 'law',      icon: Gavel,   color: 'text-emerald' },
  failed:    { label: 'Failed',          badge: 'failed',   icon: Scale,   color: 'text-against-400' },
  continued: { label: 'Continued',       badge: 'proposed', icon: Scale,   color: 'text-surface-400' },
}
...
const CHAPTER_ICON: Record<string, typeof Scale> = {
  'The Question':    Scale,
  'The Case For':    ThumbsUp,
  'The Case Against': ThumbsDown,
  'Where Things Stand': Zap,
}

const CHAPTER_ACCENT: Record<string, string> = {
  'The Question':    'border-surface-400/50 text-surface-300',
  'The Case For':    'border-for-500/50 text-for-300',
  'The Case Against': 'border-against-500/50 text-against-300',
  'Where Things Stand': 'border-gold/50 text-gold',
}

const CHAPTER_BG: Record<string, string> = {
  'The Question':    'bg-surface-300/10',
  'The Case For':    'bg-for-500/5',
  'The Case Against': 'bg-against-500/5',
  'Where Things Stand': 'bg-gold/5',
}
...
function ChapterCard({ chapter, index }: { chapter: NarrativeChapter; index: number }) {
...
export function NarrativeClient({
  ...
  const generate = useCallback(async (_force = false) => {
    if (generating) return
    setGenerating(true)
    setError(null)

    try {
      const res = await fetch(`/api/topics/${topicId}/narrative`, {
        method: 'GET',
      ...
      const json = await res.json() as NarrativeResponse & { unavailable?: boolean; insufficient_data?: boolean }
  ...
  const copyNarrative = useCallback(() => {
    ...
    const text = [
      ...
      `Via Lobby Market — lobby.market/topic/${topicId}/narrative`,
    ].join('\n')
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 pb-28 md:pb-12">
        ...
        <div className="mb-6 space-y-4">
          <div className="flex items-start gap-3">
            <div className="flex items-center justify-center h-11 w-11 rounded-xl bg-purple/10 border border-purple/30 flex-shrink-0 mt-0.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="font-mono text-xs text-purple font-semibold uppercase tracking-wider">
              Narrative Arc · AI-generated journalistic summary
            </p>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          {generating && !data && (
            <motion.div
              ...
              className="space-y-4"
            >
              <div className="flex items-center gap-2 p-4 rounded-xl bg-purple/5 border border-purple/20">
                <Loader2 className="h-4 w-4 text-purple animate-spin flex-shrink-0" />
                <p className="text-sm font-mono text-purple">
          ...
          {!generating && insufficientData && (
            ...
            >
              ...
              <p className="text-xs font-mono text-surface-500 max-w-xs mx-auto">
                This debate needs at least {3} arguments before a narrative can be written.
                <Link
                  href={`/topic/${topicId}/argue`}
                  className="block mt-2 text-for-400 hover:text-for-300 transition-colors"
                >
                  Be the first to argue →
                </Link>
              </p>
            </motion.div>
          )}
          ...
          {data && (
            ...
            >
              ...
              <motion.blockquote
                initial={{ opacity: 0, y: 8 }}
                animate={{ opacity: 1, y: 0 }}
                className="border-l-2 border-purple/60 pl-4 py-1"
              >
              ...
              <motion.div
                ...
                className="rounded-2xl bg-surface-100 border border-gold/30 p-4 text-center"
              >
                <p className="text-xs font-mono text-gold uppercase tracking-wider mb-1.5">
                  The Core Trade-off
                </p>
        ...
        <div className="mt-10 pt-6 border-t border-surface-300">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              { href: `/topic/${topicId}/steelman`, label: 'Steelman', desc: 'Strongest cases' },
              { href: `/topic/${topicId}/brief`,    label: 'Brief',     desc: 'Voter summary' },
              { href: `/topic/${topicId}/synthesis`, label: 'Synthesis',  desc: 'Points of agreement' },
              { href: `/topic/${topicId}/versus`,   label: 'Versus',    desc: 'Best vs best' },
            ].map(({ href, label, desc }) => (
              <Link
                key={href}
                href={href}

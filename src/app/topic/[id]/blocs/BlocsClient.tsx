...
import type {
  ...
  CloutBloc,
  CoalitionStance,
} from '@/app/api/topics/[id]/blocs/route'
...
export function BlocsClient({ topicId, statement, status }: BlocsClientProps) {
  const [data, setData] = useState<TopicBlocsResponse | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/topics/${topicId}/blocs`)
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="text-xs font-mono text-surface-500 uppercase tracking-wider">
                  {isActive ? 'Current Consensus' : 'Final Verdict'}
                </p>
          ...
          <div className="space-y-8">
            ...
            {data.by_role.length === 0 && data.by_clout.length === 0 && data.coalitions.length === 0 && (
              <EmptyState
                ...
                description="Voting bloc analysis requires vote data linked to user profiles. Check back as more people weigh in."
              />
            )}
            ...
            <div className="border-t border-surface-300 pt-6">
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  { href: `/topic/${topicId}/voters`, label: 'Top Voters' },
                  { href: `/topic/${topicId}/scorecard`, label: 'Scorecard' },
                  { href: `/topic/${topicId}/connections`, label: 'Connections' },
                  { href: `/topic/${topicId}/reasons`, label: 'Vote Reasons' },
                ].map(({ href, label }) => (
                  <Link
                    key={href}
                    href={href}

...
// ─── HotTakeCard (topic-specific: no topic link) ─────────────────────────────
...
export default function TopicHotTakesPage() {
  ...
  // Capture topic info from the first successful response
  const topicRef = useRef<HotTake['topic'] | null>(null)
  const [topic, setTopic] = useState<HotTake['topic'] | null>(null)

  const fetchTakes = useCallback(
    async (isSince = false) => {
      try {
        const params = new URLSearchParams({ topic_id: topicId, limit: '40', side })
        ...
        // Grab topic metadata from first take
        if (!topicRef.current && data.takes.length > 0 && data.takes[0].topic) {
          topicRef.current = data.takes[0].topic
          setTopic(data.takes[0].topic)
        }
    ...
    <div className="flex flex-col min-h-screen bg-surface-100">
      ...
      <main className="flex-1 pb-20 md:pb-0">
        <div className="max-w-2xl mx-auto px-4 pt-6">
          ...
          <div className="flex items-start gap-3 mb-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <h1 className="text-lg font-bold text-white tracking-tight">Hot Takes</h1>
                {topic && (
                  <Badge variant={STATUS_BADGE[topic.status] ?? 'proposed'} className="flex-shrink-0">
          ...
          {/* Vote split bar */}
          {topic && topic.total_votes > 0 && (
            <div className="mb-5 rounded-xl bg-surface-200/60 border border-surface-300/40 px-4 py-3">
              <div className="flex justify-between text-[11px] font-mono mb-1.5">
                <span className="text-for-400 font-bold">FOR {Math.round(topic.blue_pct)}%</span>
                <span className="text-surface-500">{topic.total_votes.toLocaleString()} votes</span>
                <span className="text-against-400 font-bold">AGAINST {100 - Math.round(topic.blue_pct)}%</span>
              </div>
            ...
            <EmptyState
              ...
              description={
                side !== 'all'
                  ...
                  : 'Be the first — cast your vote and share your hot take.'
              }
              actions={[
                { label: 'Cast a vote', href: `/topic/${topicId}` },
                { label: 'All hot takes', href: '/hot-takes' },
              ]}
            />
          ) : (
            <div className="space-y-3">
          ...
          {!loading && !error && (
            <div className="mt-8 mb-4 flex items-center justify-center gap-4 text-xs font-mono text-surface-500 flex-wrap">
              ...
              <Link href={`/topic/${topicId}/reasons`} className="hover:text-white transition-colors">
                Vote reasons
              </Link>
              <Link href={`/topic/${topicId}/voters`} className="hover:text-white transition-colors">
                Voter list
              </Link>

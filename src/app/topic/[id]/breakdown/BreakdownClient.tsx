...
/**
 * /topic/[id]/breakdown — Voter Cohort Breakdown
 *
 * Shows the vote split across four independent cohort dimensions:
 *   • Role tier (Citizen, Debater, Moderator, Elder)
 *   • Clout tier (Newcomer → Elite)
 *   • Engagement type (Argument authors vs Silent voters)
 *   • Timing cohort (Early adopters / Majority wave / Late arrivals)
 *
 * Distinct from:
 *   /topic/[id]/voters       — individual voter profiles with pagination
 *   /topic/[id]/swing        — archetype-based persuasion gap analysis
 *   /topic/[id]/pressure     — social-pressure & clout-weighted consensus
 *   /topic/[id]/heat         — when votes happened (temporal heatmap)
 */
...
import type { BreakdownResponse, CohortSlice } from '@/app/api/topics/[id]/breakdown/route'
...
export function BreakdownClient({
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/topics/${topicId}/breakdown`)
  ...
  const topicAvg = data?.topic.blue_pct ?? 50
        ...
        <div className="max-w-2xl mx-auto px-4 pt-4 pb-24 space-y-5">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                <BarChart2 className="w-4 h-4 text-for-400 flex-shrink-0" />
                <h1 className="text-base font-bold text-white leading-snug">
                  Voter Breakdown
                </h1>
                {topicCategory && (
                  <Badge variant="category" size="sm">{topicCategory}</Badge>
                )}
              </div>
            ...
            {data && (
              <div className="flex items-center gap-3 px-3 py-2 rounded-lg bg-surface-200/50 border border-surface-300/40">
                ...
                <span className="text-xs text-surface-500">
                  Platform average:{' '}
                  <span className="text-for-400 font-semibold">{Math.round(topicAvg)}%</span>
                  {' '}FOR across{' '}
                  <span className="text-white font-semibold">{data.topic.total_votes.toLocaleString()}</span>
                  {' '}votes
                </span>
              </div>
            )}
          ...
          <div className="text-xs text-surface-500">
            {activeTab === 'role' && 'How each user role tier votes on this topic.'}
            {activeTab === 'clout' && 'Vote split by Clout earned — do high-rep members lean differently?'}
            {activeTab === 'engagement' && 'Argument authors vs silent voters — do the talkers vote differently from the lurkers?'}
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-surface-500 text-sm">
              Not enough votes to show this breakdown yet.
            </div>
          ) : (
            <div className="space-y-3">
              {slices.map((slice, i) => (
          ...
          {!loading && slices.length > 0 && (
            ...
            >
              ...
              <div className="flex items-center gap-4 text-[10px] text-surface-500">
                ...
                <span>+/- delta vs topic average</span>
              </div>
            </motion.div>
          )}
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              { href: `/topic/${topicId}/voters`,  label: 'Individual voters', icon: Users },
              { href: `/topic/${topicId}/swing`,   label: 'Persuasion gaps',   icon: Zap   },
              { href: `/topic/${topicId}/pressure`,label: 'Social pressure',   icon: Trophy },
              { href: `/topic/${topicId}/heat`,    label: 'Vote heatmap',      icon: BarChart2 },

import { TopBar } from '@/components/layout/TopBar'
import { BottomNav } from '@/components/layout/BottomNav'

function Skel({ className }: { className?: string }) {
  return <div className={`animate-pulse rounded-lg bg-surface-300/50 ${className ?? ''}`} />
}

export default function ConsensusShiftLoading() {
  return (
    <div className="min-h-screen bg-surface-50">
      <TopBar />
      <main className="max-w-2xl mx-auto px-4 pt-6 pb-24 md:pb-12 space-y-4">
        <div className="flex items-center gap-3">
          <Skel className="h-9 w-9 rounded-xl" />
          <Skel className="h-11 w-11 rounded-xl" />
          <div className="space-y-1.5">
            <Skel className="h-6 w-40" />
            <Skel className="h-3 w-52" />
          </div>
        </div>
        {/* Window toggle skeleton */}
        <div className="flex gap-2">
          {[0, 1, 2].map((i) => <Skel key={i} className="h-9 w-20 rounded-xl" />)}
        </div>
        {/* Tab switcher skeleton */}
        <div className="flex gap-2">
          {[0, 1].map((i) => <Skel key={i} className="h-9 flex-1 rounded-xl" />)}
        </div>
        {/* Topic list skeleton */}
        <div className="space-y-3">
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className="rounded-2xl bg-surface-100 border border-surface-300 p-5 space-y-3">
              <div className="flex items-start gap-3">
                <Skel className="h-8 w-8 rounded-lg flex-shrink-0" />
                <div className="flex-1 space-y-2">
                  <Skel className="h-4 w-full max-w-sm" />
                  <Skel className="h-3 w-24" />
                </div>
                <Skel className="h-6 w-14 rounded-full flex-shrink-0" />
              </div>
              <Skel className="h-2 w-full rounded-full" />
            </div>
          ))}
        </div>
      </main>
      <BottomNav />
    </div>
  )
}

...
function CalibrationCurve({ curve }: { curve: BucketPoint[] }) {
  ...
  return (
    ...
    >
      ...
      {withData.map((b) => (
        <g key={b.label}>
          <circle
            cx={toX(b.predicted)}
            cy={toY(b.actual)}
            r={Math.max(3, Math.min(6, 3 + b.count / 10))}
            fill="#60a5fa"
            opacity="0.8"
          />
          <circle
            cx={toX(b.predicted)}
            cy={toY(b.actual)}
            r={Math.max(3, Math.min(6, 3 + b.count / 10)) + 3}
            fill="transparent"
          />
        </g>
      ))}
      ...
      {/* Axis labels */}
      <text x={CURVE_W / 2} y={CURVE_H - 2} textAnchor="middle" fontSize="8" fill="#6b7280" fontFamily="monospace">
        Predicted outcome %
      </text>
      <text
        x={8}
  ...
  return (
    <div className="flex items-center gap-3 py-2">
      <div className={cn('text-xs font-mono font-semibold w-28 flex-shrink-0', catColor(cat.category))}>
        {cat.category}
        {isBest && (
          <span className="ml-1 text-[9px] text-emerald bg-emerald/10 px-1 py-0.5 rounded">best</span>
        )}
        {isWorst && (
          <span className="ml-1 text-[9px] text-against-400 bg-against-500/10 px-1 py-0.5 rounded">weakest</span>
        )}
      </div>
      ...
      <span className={cn('text-xs font-mono font-bold tabular-nums w-10 text-right flex-shrink-0', grade.text)}>
        {cat.accuracy}%
      </span>
      <span className="text-[10px] font-mono text-surface-500 w-16 text-right flex-shrink-0">
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            >
              ...
              >
                <div className={cn(
                  'h-20 w-20 rounded-xl border-2 flex items-center justify-center flex-shrink-0',
                  gs.border, gs.ring, 'ring-4'
                )}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={cn('text-xs font-mono font-semibold uppercase tracking-wider', gs.text)}>
                      Calibration Grade
                    </span>
                  </div>
                  ...
                  <p className="text-xs font-mono text-surface-500 mt-1">
                    Based on {data.totalResolved} resolved vote{data.totalResolved !== 1 ? 's' : ''}
                  </p>
                </div>
              </motion.div>
                ...
                {[
                  {
                    ...
                    color: data.accuracy >= 60 ? 'text-emerald' : data.accuracy >= 45 ? 'text-gold' : 'text-against-400',
                  },
                  {
                    label: 'Resolved',
                  ...
                  },
                  {
                    ...
                    unit: '',
                    sub: '0 = perfect',
                    color: data.brierScore < 0.2 ? 'text-emerald' : data.brierScore < 0.25 ? 'text-gold' : 'text-against-400',
                  },
                  {
                    label: 'Best Category',
                    value: data.bestCategory ?? '—',
                    unit: '',
                    sub: data.bestCategory ? `${data.byCategory.find(c => c.category === data.bestCategory)?.accuracy ?? 0}% accuracy` : 'vote more',
                    color: catColor(data.bestCategory ?? ''),
                  },
                ].map((stat) => (
                  <div
                    key={stat.label}
              ...
              >
                <div className="rounded-xl bg-surface-100 border border-surface-300 p-4">
                  ...
                  <div className="text-2xl font-mono font-bold text-purple tabular-nums mb-1">
                    {data.contrarian.accuracy}%
                  </div>
                  <div className="h-1.5 rounded-full bg-surface-300 overflow-hidden mb-2">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${data.contrarian.accuracy}%` }}
                      transition={{ duration: 0.6, delay: 0.3 }}
                      className="h-full rounded-full bg-purple"
                    />
                  </div>
                  <div className="text-[10px] font-mono text-surface-500">
                    {data.contrarian.correct}/{data.contrarian.total} when going against the crowd
                  </div>
                </div>

                <div className="rounded-xl bg-surface-100 border border-surface-300 p-4">
                  <div className="flex items-center gap-1.5 text-[10px] font-mono text-surface-500 uppercase tracking-wider mb-3">
                    <Target className="h-3 w-3 text-for-400" />
                    Consensus Votes
                  </div>
                  <div className="text-2xl font-mono font-bold text-for-400 tabular-nums mb-1">
                    {data.consensus.accuracy}%
                  </div>
                  <div className="h-1.5 rounded-full bg-surface-300 overflow-hidden mb-2">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${data.consensus.accuracy}%` }}
                      transition={{ duration: 0.6, delay: 0.3 }}
                      className="h-full rounded-full bg-for-500"
                    />
                  </div>
                  <div className="text-[10px] font-mono text-surface-500">
                    {data.consensus.correct}/{data.consensus.total} when voting with the majority
                  </div>
                </div>
              </motion.div>
              ...
              {biasInfo && (
                ...
                >
                  <div className={cn('flex items-center justify-center h-9 w-9 rounded-lg flex-shrink-0',
                    data.majorityBias > 10 ? 'bg-against-500/10' : data.majorityBias < -10 ? 'bg-for-500/10' : 'bg-emerald/10'
                  )}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className={cn('text-sm font-mono font-semibold', biasInfo.color)}>
                      {biasInfo.text}
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="text-lg font-mono font-bold text-white tabular-nums">
                      {data.majorityBias > 0 ? '+' : ''}{data.majorityBias}
                    </div>
              ...
              >
                ...
                <p className="text-[10px] font-mono text-surface-500 mt-2 text-center">
                  Dots on the dashed diagonal = perfectly calibrated
                </p>
              </motion.div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-3 text-[10px] font-mono text-surface-500">
                      <span className="flex items-center gap-1">
                        <CheckCircle2 className="h-3 w-3 text-emerald" /> = correct
                      </span>
                      <span className="flex items-center gap-1">
                        <XCircle className="h-3 w-3 text-against-400" /> = incorrect
                      </span>
                    </div>
                  </div>
              ...
              >
                <Link
                  ...
                  className={cn(
                    ...
                    'text-xs font-mono font-medium hover:bg-purple/20 transition-colors'
                  )}
                >
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    'text-xs font-mono font-medium hover:bg-for-500/20 transition-colors'
                  )}
                >
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    'text-xs font-mono hover:border-surface-400 hover:text-white transition-colors'
                  )}
                >

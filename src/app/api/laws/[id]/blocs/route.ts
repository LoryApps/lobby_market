...
export interface BlocsResponse {
  law_id: string
...
export async function GET(
  ...
  try {
    ...
    const { data: law, error: lawErr } = await supabase
      .from('laws')
      .select('id, statement, topic_id, blue_pct, total_votes, category')
      .eq('id', params.id)
      .maybeSingle()
    ...
    if (!law.topic_id) {
      ...
      } satisfies BlocsResponse)
    }

    // ── 2. Fetch all votes for this topic with profile data ─────────────────
    // Fetch in batches to handle large vote counts
    const PAGE = 1000
    ...
    while (hasMore) {
      ...
      if (!batch || batch.length === 0) {
        ...
        from += PAGE
        // Safety cap: stop at 10k votes to avoid excessive load
    ...
    // Get unique user_ids of people who wrote arguments on this topic
    ...
    if (argRows && argRows.length > 0) {
      ...
      // Fetch votes again but with user_id to check against arguers

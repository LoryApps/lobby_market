...
export async function GET() {
  ...
  //    We compute this by looking at topics that have each tag, averaging blue_pct weighted by total_votes.
  //    Only fetch for tags the user has voted on.
  const userTags = Array.from(tagMap.keys())

  const communityForPct: Record<string, number> = {}

  if (userTags.length > 0) {
    // Use the tags array containment operator to find topics with each user tag.
    // We batch this by fetching all topics that have ANY of the user's tags, then aggregate client-side.
    ...
    if (communityTopics) {
      // Aggregate weighted FOR% per tag
  ...
  for (const [tag, entry] of tagMap) {
    ...
    // Top topic = one with most votes in this tag
  ...
  // Most contrarian tag = largest |alignment_delta|

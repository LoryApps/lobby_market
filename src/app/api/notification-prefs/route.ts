...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select(
      'achievement_earned, debate_starting, law_established, topic_activated, vote_threshold, reply_received, role_promoted, lobby_update, new_topic_in_tag, streak_reminder, weekly_digest, qa_notifications, ama_notifications, relay_notifications, debate_challenge_notifications'
    )
    .eq('user_id', user.id)
    .maybeSingle()

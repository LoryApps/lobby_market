...
/**
 * /exchange/[id]/commentary — Per-market commentary feed
 *
 * Same Twitter-style micro-notes as /exchange/commentary but scoped to a
 * single civic prediction market. Compose box auto-links posts to this topic.
 */
...
// ─── Market summary passed from server ───────────────────────────────────────
...
function statusColor(status: string): string {
  if (status === 'law') return 'bg-gold/20 text-gold border-gold/30'
  if (status === 'failed') return 'bg-against-500/20 text-against-400 border-against-500/30'
  if (status === 'voting') return 'bg-purple/20 text-purple border-purple/30'
  return 'bg-for-500/10 text-for-400 border-for-500/30'
}
  ...
  return (
    ...
    >
            ...
            {dirCfg && (
              <span
                className={cn(
                  ...
                  dirCfg.bg, dirCfg.border, dirCfg.color
                )}
              >
      ...
      <div className="flex items-center justify-between mt-3 pt-2.5 border-t border-surface-300/40">
        ...
        >
          <Heart className={cn('h-3.5 w-3.5 transition-all', liked && 'fill-current')} aria-hidden="true" />
  ...
  if (!currentUserId) {
    return (
      <div className="bg-surface-100 border border-surface-300/60 rounded-xl p-4 text-center">
        ...
        <p className="text-sm text-surface-500 mb-3">Sign in to share your take on this market</p>
        <Link href="/sign-in">
    ...
    <div className="bg-surface-100 border border-surface-300/60 rounded-xl p-4">
      <textarea
        ...
        className={cn(
          ...
          'resize-none outline-none leading-relaxed border-0 p-0 min-h-[72px]'
        )}
      />
      ...
      {error && <p className="mt-2 text-xs text-against-400">{error}</p>}
    </div>
  )
}
...
function MarketHeader({ market }: { market: MarketSummary }) {
  const price = Math.round(market.blue_pct ?? 50)

  return (
    <Link
      href={`/exchange/${market.id}`}
      className="block bg-surface-100 border border-surface-300/60 rounded-xl p-4 hover:border-surface-400/60 transition-colors group mb-5"
    >
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-2">
            ...
            <span
              className={cn(
                'inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-mono font-semibold border uppercase tracking-wide',
          ...
          <p className="text-sm font-medium text-surface-900 group-hover:text-for-400 transition-colors leading-snug line-clamp-2">
            {market.statement}
          </p>
        </div>

        <div className="flex-shrink-0 text-right">
          <div className={cn('text-2xl font-bold tabular-nums font-mono', priceColor(price))}>
            {price}<span className="text-sm">¢</span>
          </div>
          {market.total_votes != null && (
            <div className="text-[11px] text-surface-500 font-mono mt-0.5">
              {market.total_votes.toLocaleString()} votes
            </div>
          )}
        </div>
      </div>

      <div className="flex items-center gap-1 mt-2 text-[11px] text-surface-500 font-mono">
        <ExternalLink className="h-3 w-3" aria-hidden="true" />
        <span className="group-hover:text-surface-400 transition-colors">View market</span>
      </div>
    </Link>
  )
}
...
export function MarketCommentaryClient({
  id,
  market,
}: {
  id: string
  ...
  return (
    <div className="min-h-screen bg-surface-50">
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4 pb-24">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-surface-500 font-mono mt-0.5">
              {total > 0 ? `${total.toLocaleString()} notes` : 'Trader takes on this market'}
            </p>
          </div>
          <button
        ...
        {/* Compose box */}
        <div className="mb-5">
          <ComposeBox topicId={id} currentUserId={currentUserId} onPosted={handlePosted} />
        </div>

        {/* Filters */}
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          <div className="flex bg-surface-200/60 rounded-lg p-0.5 gap-0.5">
            {SORT_TABS.map(({ id: sid, label, icon: Icon }) => (
              <button
                key={sid}
                onClick={() => setSort(sid)}
                className={cn(
          ...
          <div className="flex gap-1.5 flex-wrap">
            {DIR_FILTERS.map(({ id: did, label }) => (
              <button
                key={String(did)}
                onClick={() => setDirFilter(did as DirFilter)}
                className={cn(
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description={
              dirFilter
                ? `No ${dirFilter} takes on this market. Be the first.`
                : 'No notes on this market yet. Share your first take above.'
            }
          />
        ) : (
        ...
        {!hasMore && notes.length > 0 && !loading && (
          <p className="mt-6 text-center text-[11px] text-surface-600 font-mono">
            — {total.toLocaleString()} {total === 1 ? 'note' : 'notes'} on this market —
          </p>
        )}
      </main>

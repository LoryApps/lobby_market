...
import type { WatchingThesisResponse } from '@/app/api/thesis/watching/route'
...
function ThesisCard({
  ...
  const days = thesis.resolution_date ? calcDaysLeft(thesis.resolution_date) : null
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 px-4 pt-4 pb-0">
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <span className={cn('text-[10px] font-mono font-semibold px-2 py-0.5 rounded-full border capitalize', catColor)}>
            {thesis.category}
          </span>
          <span className={cn('text-[10px] font-mono font-semibold', statusCfg.color)}>
      ...
      {total > 0 && (
        <div className="px-4 mb-3 space-y-1">
          <div className="flex h-1.5 rounded-full overflow-hidden bg-surface-300">
            <div className="bg-for-500 transition-all duration-500" style={{ width: `${agreeWidth}%` }} />
            <div className="bg-against-500 transition-all duration-500" style={{ width: `${100 - agreeWidth}%` }} />
          </div>
        ...
        <div className="flex border-t border-surface-300">
          <button
            ...
            className={cn(
              ...
              'border-r border-surface-300 transition-all focus:outline-none disabled:opacity-50',
              localVote === true
...
export function WatchingThesesClient() {
  ...
  const fetchTheses = useCallback(async (reset = true) => {
    ...
    try {
      ...
      const res = await fetch(`/api/thesis/watching?${params}`, { cache: 'no-store' })
      if (!res.ok) return

      const data: WatchingThesisResponse = await res.json()
    ...
    return (
      <div className="min-h-screen bg-surface-50">
        ...
        <main className="max-w-2xl mx-auto px-4 py-16 pb-24 flex flex-col items-center justify-center text-center gap-4">
          <Bell className="h-10 w-10 text-surface-500" />
          <p className="text-white font-mono text-lg font-bold">Sign in to view your thesis watchlist</p>
          <Link
            href="/sign-in"
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 pb-24 md:pb-12">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="font-mono text-xl font-bold text-white">Thesis Watchlist</h1>
              <p className="text-xs font-mono text-surface-500 mt-0.5">
                Predictions you are tracking
              </p>
            </div>
          </div>
          <button
          ...
          {loading ? (
            ...
            <motion.div key="empty" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
              <EmptyState
                ...
                description={
                  statusTab !== 'all'
                    ? `No ${statusTab} theses in your watchlist. Switch to All to see everything you're tracking.`
                    : 'Watch individual theses to track them here. Click the bell icon on any thesis to add it to your watchlist.'
                }
            ...
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              <p className="text-xs font-mono text-surface-500">
                {total} {total === 1 ? 'thesis' : 'theses'} in your watchlist
              </p>
              {theses.map((thesis, i) => (

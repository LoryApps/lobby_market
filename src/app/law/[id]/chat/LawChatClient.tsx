...
interface LawChatClientProps {
  law: Law
  initialMessages: LawChatMessage[]
  currentUserId: string | null
}

export function LawChatClient({
  law,
  initialMessages,
  currentUserId,
}: LawChatClientProps) {
  const [messages, setMessages] = useState<LawChatMessage[]>(initialMessages)
  ...
  // Supabase Realtime: subscribe to new chat messages for this law
  useEffect(() => {
    const supabase = createClient()

    // Presence channel for online count
    const presenceChannel = supabase.channel(`law-chat-presence:${law.id}`, {
    ...
    const msgChannel = supabase
      .channel(`law-chat-messages:${law.id}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
          schema: 'public',
          table: 'law_chat_messages',
          filter: `law_id=eq.${law.id}`,
        },
        (payload) => {
          ...
          // Fetch author profile (postgres_changes doesn't return joins)
          supabase
            .from('profiles')
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/laws/${law.id}/chat`, {
        method: 'POST',
  ...
  }, [draft, sending, currentUserId, law.id])

  const handleDelete = useCallback(async (messageId: string) => {
    try {
      await fetch(`/api/laws/${law.id}/chat?messageId=${messageId}`, {
        method: 'DELETE',
      })
    } catch {
      // ignore
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <span className="font-mono text-xs font-semibold text-gold uppercase tracking-wide">
                  Law Chat
                </span>
                <MessageCircle className="h-3 w-3 text-surface-600" />
              </div>
        ...
        <div className="max-w-2xl mx-auto space-y-3">
          {messages.length === 0 ? (
            <EmptyState
              ...
              description="Be the first to discuss this law."
              className="mt-16"
            />
          ) : (
            ...
            <div className="flex items-end gap-2">
              <div className="flex-1 relative">
                <textarea
                  ...
                  placeholder="Discuss this law… (Enter to send)"
                  rows={1}
                  ...
                  className={cn(
                    ...
                    isOverLimit
                      ? 'border-against-500'
                      : 'border-surface-300 focus:border-gold/50',
                  )}
              ...
              <button
                ...
                className={cn(
                  ...
                  draft.trim() && !isOverLimit && !sending
                    ? 'bg-gold/20 hover:bg-gold/30 border border-gold/40 text-gold'
                    : 'bg-surface-200 text-surface-600 cursor-not-allowed border border-surface-300',
                )}

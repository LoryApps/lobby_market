...
import type { LawQuizData, LawQuizQuestion } from '@/app/api/laws/[id]/quiz/route'
...
function scoreBadge(score: number, total: number) {
  ...
  if (pct === 1) return { label: 'Codex Master!', color: 'text-gold' }
  if (pct >= 0.8) return { label: 'Law Scholar', color: 'text-for-400' }
  if (pct >= 0.6) return { label: 'Civic Citizen', color: 'text-emerald' }
  if (pct >= 0.4) return { label: 'Still Learning', color: 'text-surface-600' }
...
function OptionButton({
  ...
  if (revealed) {
    if (isCorrect) {
      ...
      bg = 'bg-gold/10'
      textColor = 'text-gold'
      icon = <CheckCircle2 className="h-4 w-4 flex-shrink-0 text-gold" />
      ...
      {Array.from({ length: total }).map((_, i) => (
        <div
          ...
          className={cn(
            ...
            i < current ? 'bg-gold' : i === current ? 'bg-gold/40' : 'bg-surface-200',
          )}
        />
      ))}
    </div>
...
export function LawQuizClient({ lawId }: { lawId: string }) {
  const [quiz, setQuiz] = useState<LawQuizData | null>(null)
  ...
  const fetchQuiz = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/laws/${lawId}/quiz`)
      if (!res.ok) throw new Error('Failed to load quiz')
      const data: LawQuizData = await res.json()
      setQuiz(data)
    } catch {
  ...
  const currentQuestion: LawQuizQuestion | undefined = quiz?.questions[questionIndex]
  ...
  const estYear = quiz?.establishedAt
    ? new Date(quiz.establishedAt).getFullYear()
    : null
    ...
    return (
      <div className="flex min-h-screen flex-col bg-surface-50">
        ...
        <main className="mx-auto w-full max-w-lg flex-1 px-4 py-16 text-center">
          <Gavel className="mx-auto mb-4 h-12 w-12 text-surface-500" />
          <p className="mb-6 text-surface-700">{error ?? 'Quiz unavailable.'}</p>
          <button
            onClick={fetchQuiz}
            className="rounded-xl bg-gold px-6 py-3 text-sm font-semibold text-black hover:bg-gold/90"
          >
            Retry
          </button>
        </main>
            ...
            >
              <div className="rounded-2xl border border-gold/30 bg-gold/5 p-6">
                <div className="mb-3 flex items-center gap-2">
                  ...
                  {estYear && (
                    <span className="ml-auto rounded-full bg-gold/10 px-2.5 py-0.5 text-xs font-medium text-gold">
                      Est. {estYear}
                    </span>
                  )}
                </div>
              ...
              <div className="space-y-3 text-sm text-surface-700">
                ...
                <ul className="space-y-1.5 pl-4">
                  <li className="flex items-start gap-2">
                    ...
                    {quiz.questions.length} questions drawn from this law&apos;s real data
                  </li>
                  <li className="flex items-start gap-2">
                    <ChevronRight className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-gold" />
                    Each answer reveals an explanation from the Codex
                  </li>
                  <li className="flex items-start gap-2">
                    <ChevronRight className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-gold" />
              ...
              <button
                ...
                className="w-full rounded-xl bg-gold py-3.5 text-sm font-semibold text-black shadow-md hover:bg-gold/90 active:scale-95 transition-all"
              >
                Start Quiz
              </button>
            </motion.div>
          ...
          {phase === 'results' && (
            ...
            >
              ...
              <div className="flex gap-3">
                ...
                <Link
                  href={`/law/${lawId}`}
                  className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-gold py-3 text-sm font-semibold text-black hover:bg-gold/90 transition-all"
                >
                  Back to Law
                </Link>
              </div>
              ...
              <div className="rounded-xl border border-surface-200 bg-surface-100 p-4">
                <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-surface-600">
                  Explore the Codex
                </p>
                <div className="flex flex-col gap-2">
                  <Link
                    href="/law"
                    className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-surface-700 hover:bg-surface-200 transition-colors"
                  >
                    <Gavel className="h-4 w-4 text-gold" />
                    Browse all established laws
                  </Link>
                  <Link
                    href={`/law/${lawId}/wiki`}
                    className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-surface-700 hover:bg-surface-200 transition-colors"

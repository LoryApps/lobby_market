...
function ClashRing({ clashScore }: { clashScore: number }) {
  ...
  const color =
    clashScore >= 80 ? '#ef4444' :
    clashScore >= 65 ? '#f87171' :
    clashScore >= 50 ? '#fb923c' :
                       '#f59e0b'
      ...
      <div className="rounded-2xl border border-surface-300/40 bg-surface-100 hover:border-against-500/30 transition-colors group">
        ...
        <AnimatePresence>
          {expanded && rival.flashpoints.length > 0 && (
            ...
            >
              <div className="px-4 pb-4 pt-3 space-y-2 border-t border-surface-300/30">
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401) {
          ...
          setError((body as { error?: string }).error ?? 'Failed to load rivals.')
        }
        return
      }
